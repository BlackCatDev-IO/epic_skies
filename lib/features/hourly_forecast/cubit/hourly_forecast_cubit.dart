...
class HourlyForecastCubit extends HydratedCubit<HourlyForecastState> {
  ...
  HourlyForecastState _sortedHourlyState(
    ...
    // Sort each day list by forecastStart time (ascending)
    ...
    // Return the SortedHourlyList object
  ...
  HourlyForecastState _sortHourlyConditions(
    List<HourlyForecastModel> conditions,
  ) {
    ...
    for (final condition in conditions) {
      if (condition.time.isBefore(currentTime)) {
        continue;
      }

      final daysDifference = condition.time.difference(currentTime).inDays;

      if (daysDifference == 0 &&
          next24Hours.length < 24 &&
          condition.time.isAfter(currentTime)) {
        next24Hours.add(condition);
      }

      if (condition.time.isTomorrow) {
        day1.add(condition);
      }
  ...
  List<HourlyForecastModel> _initHourlyWeatherKitData() {
    ...
    for (var i = 0; i <= hourlyList.length - 1; i++) {
      ...
      final startTime =
          _timezoneUtil.addedTimezoneOffset(hourlyWeatherKitData.forecastStart);

      final test = hourlyWeatherKitData.forecastStart;
      final testOffset = _timezoneUtil.addedTimezoneOffset(test);
      final testToLocal = test.toLocal();
